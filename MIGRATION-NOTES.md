...
| Every commit runs build + push | **Split path filter:** workflow runs on app/chart/IaC/ArgoCD changes; **Semgrep** on app or static paths; **build/push/Trivy/cosign** only when `app/quote-api/**` changes | Docs/scripts edits do not rebuild the image; chart/IaC edits still get SAST. Deploy stays GitOps-side (ArgoCD syncs the chart; CI never runs `kubectl`). |
